...
test('notifications page renders real inbox actions and empty state copy', () => {
    ...
    assert.match(pageSource, /destroyNotification\.url/);
    assert.match(pageSource, /router\.delete/);
    assert.match(pageSource, /confirmNotificationDeletion/);
    assert.match(pageSource, /Trash2/);
    assert.match(pageSource, /DialogContent/);
    assert.match(pageSource, /actions\.delete/);
    assert.match(pageSource, /aria-disabled="true"/);
    assert.doesNotMatch(pageSource, /link\.url \|\| '#'/);
});
