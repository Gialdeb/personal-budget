...
test('recurring form uses public uuids instead of database ids for selected entities', () => {
    ...
    assert.doesNotMatch(
        formSource,
        /category_id:\s*Number\(form\.category_id\)/,
    );
...
test('recurring pages close the sheet on saved and use the updated convert label', () => {
    ...
    assert.match(
        showSource,
        /transactions\.recurring\.dialogs\.convertFutureTitle/,
    );
    assert.match(
        showSource,
        /transactions\.recurring\.dialogs\.undoConversionTitle/,
    );
...
test('recurring form surfaces required-field validation and enforces end date after start date in the UI', () => {
    assert.match(
        formSource,
        /transactions\.recurring\.form\.errors\.accountRequired/,
    );
    assert.match(
        formSource,
        /transactions\.recurring\.form\.errors\.categoryRequired/,
    );
    assert.match(
        formSource,
        /transactions\.recurring\.form\.errors\.startDateRequired/,
    );
    assert.match(
        formSource,
        /transactions\.recurring\.form\.errors\.endDateBeforeStartDate/,
    );
...
test('recurring form filters category scope and tracked item options by the selected account contributors', () => {
    ...
    assert.match(formSource, /full_path: option\.full_path \?\? option\.label/);
    assert.match(formSource, /ancestor_uuids: option\.ancestor_uuids \?\? \[]/);
    assert.match(formSource, /hierarchical/);
    assert.match(formSource, /is_selectable/);
    assert.match(
        formSource,
        /watch\(\s*\(\) => form\.category_uuid,\s*\(\) => \{\s*ensureTrackedItemMatchesAccountContext\(\);/s,
    );
});
...
test('recurring account selectors use visual ownership badges instead of the old plain Mio suffix', () => {
    assert.match(
        formSource,
        /transactions\.recurring\.form\.accountBadges\.owner/,
    );
    assert.match(
        formSource,
        /transactions\.recurring\.form\.accountBadges\.shared/,
    );
