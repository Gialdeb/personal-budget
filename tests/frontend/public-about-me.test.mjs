...
test('about me page keeps mobile responsive hero and profile layout safeguards', () => {
    ...
    assert.match(
        aboutSource,
        /class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-\[#ea5a47].*sm:w-auto/,
    );
    assert.match(
        aboutSource,
    ...
    assert.match(
        aboutSource,
        /class="h-56 w-full rounded-\[1\.5rem] object-cover object-\[center_18%].*sm:size-20 sm:w-20 sm:object-cover sm:object-top"/,
    );
    assert.match(
        aboutSource,
        /text-sm leading-7 break-all text-slate-600 sm:break-normal/,
    );
});
