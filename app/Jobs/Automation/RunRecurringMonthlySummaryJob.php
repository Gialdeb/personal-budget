...
class RunRecurringMonthlySummaryJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        RecurringSummaryNotificationService $summaryNotificationService,
    ): void {
        $runner->run(
            automationKey: 'recurring_monthly_summary',
            pipeline: 'recurring_monthly_summary',
            triggerType: $this->triggerType,
            callback: function () use ($summaryNotificationService): array {
                $referenceDate = $this->referenceDate
                    ? CarbonImmutable::parse($this->referenceDate)
                    : null;
                $result = $summaryNotificationService->sendMonthlySummary($referenceDate);
                ...
                return [
                    ...
                    'message' => $errorCount > 0
                        ? __('admin.automation.recurringSummaries.partialFailure')
                        : null,
                ];
            },
            jobClass: self::class,
