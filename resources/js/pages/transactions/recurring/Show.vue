...
import { ArrowLeft, Pause, Pencil, Play, Receipt, ShieldCheck, Undo2 } from 'lucide-vue-next';
import { computed, ref } from 'vue';
import { useI18n } from 'vue-i18n';
import { cancel, pause, resume } from '@/actions/App/Http/Controllers/RecurringEntryController';
...
function formatMoney(value: number, currency?: string | null): string {
    return formatCurrency(
        value,
        currency ?? entry.value.currency ?? auth.value.user?.base_currency_code ?? 'EUR',
...
function occurrenceState(occurrence: (typeof props.recurringEntry.occurrences)[number]): {
    label: string;
    tone: string;
} {
...
function handleCancel(): void {
    router.patch(cancel.url(entry.value.uuid), {}, {
        preserveScroll: true,
        onFinish: () => {
...
function handleConvert(occurrenceUuid: string): void {
    router.post(convertOccurrence.url([entry.value.uuid, occurrenceUuid]), {
...
function requestConvert(occurrence: (typeof props.recurringEntry.occurrences)[number]): void {
...
function handleRefund(): void {
    ...
    router.post(refundTransaction.url(refundDialogTransactionUuid.value), {}, {
        preserveScroll: true,
        onFinish: () => {
...
function handleUndoConversion(): void {
    ...
    router.delete(`/recurring-entries/${entry.value.uuid}/occurrences/${undoConversionOccurrenceUuid.value}/conversion`, {
        preserveScroll: true,
        onFinish: () => {
...
function occurrenceDateValue(occurrence: (typeof props.recurringEntry.occurrences)[number]): string | null {
    return occurrence.due_date ?? occurrence.expected_date;
}

function isFutureOccurrence(occurrence: (typeof props.recurringEntry.occurrences)[number]): boolean {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 px-4 py-5 sm:px-6 lg:px-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300">
                                {{ entry.description ?? t('transactions.recurring.labels.noDescription') }}
                            </p>
                        </div>

                        <div class="flex flex-wrap gap-2">
                            <Button class="rounded-2xl" @click="formOpen = true">
            ...
            <section class="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/92 shadow-sm dark:border-white/10 dark:bg-slate-950/85">
                <div class="border-b border-slate-200/70 p-5 dark:border-white/10 lg:p-6">
                    ...
                    >
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <Button
                                ...
                                variant="outline"
                                class="h-9 rounded-full px-3 text-xs"
                                @click="undoConversionOccurrenceUuid = occurrence.uuid"
                            >
                            ...
                            <Button
                                ...
                                variant="outline"
                                class="h-9 rounded-full px-3 text-xs"
                                @click="refundDialogTransactionUuid = occurrence.converted_transaction.uuid"
                            >
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-950/[0.03] text-left text-xs uppercase tracking-[0.16em] text-slate-500 dark:bg-white/[0.04] dark:text-slate-400">
                            <tr>
                                ...
                                <th class="px-4 py-3">{{ t('transactions.recurring.labels.amount') }}</th>
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-4 py-4 font-semibold text-slate-950 dark:text-white">
                                    {{ formatMoney(occurrence.expected_amount ?? 0) }}
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex flex-wrap gap-2">
                                ...
                                <td class="px-4 py-4">
                                    <div v-if="occurrence.converted_transaction" class="space-y-1">
        ...
        <RecurringEntryFormSheet
            ...
            :default-start-date="entry.start_date ?? new Date().toISOString().slice(0, 10)"
        ...
        <Dialog v-model:open="cancelDialogOpen">
            <DialogContent class="sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle>{{ t('transactions.recurring.dialogs.cancelTitle') }}</DialogTitle>
                    <DialogDescription>
                        {{ t('transactions.recurring.dialogs.cancelDescription') }}
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button variant="outline" class="rounded-xl" @click="cancelDialogOpen = false">
                        {{ t('app.common.cancel') }}
                    </Button>
        ...
        <Dialog :open="convertDialogOccurrenceUuid !== null" @update:open="(value) => { if (!value) convertDialogOccurrenceUuid = null; }">
            <DialogContent class="sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle>{{ t('transactions.recurring.dialogs.convertFutureTitle') }}</DialogTitle>
                    <DialogDescription>
                        {{ t('transactions.recurring.dialogs.convertFutureDescription') }}
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button variant="outline" class="rounded-xl" @click="convertDialogOccurrenceUuid = null">
                        {{ t('app.common.cancel') }}
                    </Button>
                    <Button class="rounded-xl" @click="handleConvert(convertDialogOccurrenceUuid!)">
                        {{ t('transactions.recurring.actions.convert') }}
                    </Button>
                </DialogFooter>
            </DialogContent>
        ...
        <Dialog :open="undoConversionOccurrenceUuid !== null" @update:open="(value) => { if (!value) undoConversionOccurrenceUuid = null; }">
            <DialogContent class="sm:max-w-xl">
                <DialogHeader>
                    <DialogTitle>{{ t('transactions.recurring.dialogs.undoConversionTitle') }}</DialogTitle>
                    <DialogDescription>
                        {{ t('transactions.recurring.dialogs.undoConversionDescription') }}
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button variant="outline" class="rounded-xl" @click="undoConversionOccurrenceUuid = null">
                        {{ t('app.common.cancel') }}
                    </Button>
        ...
        <Dialog :open="refundDialogTransactionUuid !== null" @update:open="(value) => { if (!value) refundDialogTransactionUuid = null; }">
            <DialogContent class="sm:max-w-xl">
                <DialogHeader>
                ...
                <DialogFooter>
                    <Button variant="outline" class="rounded-xl" @click="refundDialogTransactionUuid = null">
                        {{ t('app.common.cancel') }}
                    </Button>
